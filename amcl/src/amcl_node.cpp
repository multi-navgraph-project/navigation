...
private:
  ...
  void publishUnknownScan(double x, double y, double yaw);
                       ...
                       first_reconfigure_call_(true)
{
  ...
  unknownScanPub_ = nh_.advertise<sensor_msgs::LaserScan>("unknown_scan", 1, true);
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan)
{
  ...
  if (resampled || force_publication)
  {
    ...
    if (max_weight > 0.0)
    {
      ...
      // Compute unknown scans
      publishUnknownScan(hyps[max_weight_hyp].pf_pose_mean.v[0], hyps[max_weight_hyp].pf_pose_mean.v[1], normalize(hyps[max_weight_hyp].pf_pose_mean.v[2]));
...
void AmclNode::publishUnknownScan(double x1, double y1, double yaw1)
{
  unknownScan_ = scan_;
  double sensorX = x1;
  double sensorY = y1;
  double sensorYaw = yaw1;
  for (int i = 0; i < (int)unknownScan_.ranges.size(); ++i)
  {
    ...
    mj = MAP_GYWY(map_, y);
    double pKnown;
    double pKnownPrior_ = 0.5;
    double pUnknownPrior_ = 1.0 - pKnownPrior_;
    double z_hit_denom = 2 * sigma_hit_ * sigma_hit_;
    double z_rand_mult = 1.0 / unknownScan_.range_max;
    double z;
    double pUnknown = 0.01 * exp(-0.01 * r) / (1.0 - exp(-0.01 * unknownScan_.range_max)) * pUnknownPrior_;
    if (!MAP_VALID(map_, mi, mj))
      z = map_->max_occ_dist;
    else
      z = map_->cells[MAP_INDEX(map_, mi, mj)].occ_dist;

    pKnown = (z_hit_ * exp(-(z * z) / z_hit_denom) + z_rand_ * z_rand_mult) * pKnownPrior_; //(z_hit * p_hit +measurementModelRandom_)
